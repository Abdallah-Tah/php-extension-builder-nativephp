...

{
...

    {
        ...
        try {
            ...
            // STEP 1.5: Setup PHP SDK Binary Tools (Windows only)
            ...
            if ($this->isWindows()) {
                ...
                $this->info('STEP 1.5: Skipping PHP SDK Binary Tools (not needed on ' . PHP_OS_FAMILY . ')');
            }
...

    {
        ...
        $grepCmd = $this->isWindows() ? 'findstr' : 'grep';
        $versionCheck = Process::path($spcPath)->run("php bin/spc list sources | {$grepCmd} php-src");
        ...
        // CRITICAL: Pre-extract PHP source and PECL extensions (Windows only)
        // On Unix systems (macOS/Linux), static-php-cli handles extraction natively
        ...
        if ($this->isWindows()) {
            ...
            $this->info('Skipping Windows-specific extraction (not needed on ' . PHP_OS_FAMILY . ')');
        }
        ...
        $this->ensureUnixReadlineDependencies($spcPath);
...

    {
        ...
        $librariesToClean = ['bzip2', 'zlib', 'openssl', 'libssh2', 'libxml2', 'nghttp2', 'curl', 'libpng', 'sqlite', 'xz', 'libzip', 'libwebp', 'libjpeg', 'icu-static-win', 'libiconv-win', 'postgresql-win'];

        // Map to OS-specific library names
        $librariesToClean = array_map(fn($lib) => $this->getOSSpecificLibrary($lib), $librariesToClean);
...

    {
        ...
        if ($this->isWindows()) {
            ...
            if (!str_contains($path, 'C:\Program Files\Git\usr\bin')) {
                $path .= ';C:\Program Files\Git\usr\bin';
            }

            $env['PATH'] = $path;

            $env['VS_PATH'] = 'C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\VC\Tools\MSVC\14.41.34120\bin\Hostx64\x64';
            ...
            foreach ($windowsLocale as $key => $value) {
                $env[$key] = $value;
            }
        } else {
            ...
            foreach ($unixLocale as $key => $value) {
                $env[$key] = $value;
            }
        }
...

    {
        ...
        if (PHP_OS_FAMILY === 'Windows') {
            ...
            $this->validateCrossPlatformEnvironment();
        }
    ...
    protected function validateWindowsEnvironment(): void
    ...
    protected function validateCrossPlatformEnvironment(): void

    {

        $this->info('Running in cross-platform mode.');
        $this->warn('Note: This package was designed for Windows. Some features may not work on ' . PHP_OS_FAMILY . '.');
        ...
        foreach ($requiredTools as $tool) {
            ...
            if (empty($result)) {
                throw new RuntimeException("Required tool '$tool' not found. Please install it first.");
            }
        }

        $this->info('✓ Basic tools check passed.');
    }
...

    {
        ...
        $this->requiredLibraries = $this->getBaseLibraries();
        ...
        // Map Windows-specific library names to OS-specific equivalents
        $this->requiredLibraries = array_map(
            fn($lib) => $this->getOSSpecificLibrary($lib),
            $this->requiredLibraries
        );
    }

    /**
     * macOS/Linux builds need readline archives present before hitting the builder.
     * Ensure the download actually materialised so the build won't fail later.
     */
    protected function ensureUnixReadlineDependencies(string $spcPath): void
    {
        if ($this->isWindows() || !in_array('readline', $this->requiredLibraries, true)) {
            return;
        }

        if ($this->readlineIsLocked($spcPath)) {
            return;
        }

        $this->info('Ensuring readline source is available for Unix build...');

        $downloadResult = Process::path($spcPath)
            ->timeout(300)
            ->env($this->getSpcEnvironment())
            ->run('php bin/spc download readline');

        if (!$downloadResult->successful()) {
            $message = trim($downloadResult->output() . "\n" . $downloadResult->errorOutput());
            if ($message !== '') {
                $this->error($message);
            }
            throw new RuntimeException('Failed to download readline dependency required for macOS/Linux CLI builds.');
        }

        if (!$this->readlineIsLocked($spcPath)) {
            throw new RuntimeException('Readline download reported success but lock data is still missing. Please run `php bin/spc download readline --debug` manually and re-try.');
        }
    }

    protected function readlineIsLocked(string $spcPath): bool
    {
        $lockFilePath = $spcPath . '/downloads/.lock.json';
        if (!file_exists($lockFilePath)) {
            return false;
        }

        $lockData = json_decode(file_get_contents($lockFilePath), true);
        if (!is_array($lockData) || !array_key_exists('readline', $lockData)) {
            return false;
        }

        // When static-php-cli marks readline as pre-built, there may be no extracted source directory yet.
        $meta = $lockData['readline'];
        return isset($meta['hash']) && isset($meta['filename']);
    }

    /**
     * Base library set seeded before adding extension-specific requirements.
     * Windows keeps the proven list; Unix variants add ncurses so readline builds succeed.
     */
    protected function getBaseLibraries(): array
    {
        $baseLibraries = [
            ...
            'libiconv-win',   // iconv extension (Windows/macOS - will be mapped to OS-specific name)
        ];
        ...
        // Unix builds depend on curses + readline for interactive CLI binaries.
        $baseLibraries[] = 'ncurses';
        $baseLibraries[] = 'readline';
...

    {
        ...
        $criticalDeps = [
            $this->getOSSpecificLibrary('libiconv-win'),
            'freetype'
        ];
...
PYTHON;
...

    {
        ...
        if (!$zipAvailable && !$pharAvailable) {
            throw new RuntimeException('Neither ZipArchive nor PharData is available to package build artifacts.');
        }
        ...
        $legacyTar = $artifactDir . DIRECTORY_SEPARATOR . $this->buildHash . '.tar.gz';
        if (file_exists($legacyTar)) {
            @unlink($legacyTar);
        }
        ...
        if ($zipAvailable) {
            ...
            try {
                $phar = new \PharData($artifactPath, 0, null, \Phar::ZIP);
            } catch (\Exception $e) {
                throw new RuntimeException('Unable to create ZIP artifact using PharData (check that phar.readonly=0). Original error: ' . $e->getMessage(), previous: $e);
            }
    ...
    /**
     * Get OS-specific library name
     */
    protected function getOSSpecificLibrary(string $library): string
    {
        // Map Windows-specific libraries to their Unix equivalents
        $libraryMap = [
            'libiconv-win' => $this->isWindows() ? 'libiconv-win' : 'libiconv',
            'icu-static-win' => $this->isWindows() ? 'icu-static-win' : 'icu',
            'postgresql-win' => $this->isWindows() ? 'postgresql-win' : 'postgresql',
        ];
